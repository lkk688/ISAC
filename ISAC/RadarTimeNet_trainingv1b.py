    ...
    def apply_range_fft(self, x):
        ...
        if self.use_learnable_fft and self.range_fft is not None:
            ...
            if hasattr(torch, 'complex'):
                ...
                try:
                    # Use torch.fft.fft if available
...
class OFDMModulator(nn.Module):
    ...
    def forward(self, data_symbols):
        ...
        try:
            # Use torch.fft.ifft if available
            if hasattr(torch.fft, 'ifft'):
...
class ISACDataset:
    ...
    def __init__(self, 
                 ...
                 bandwidth: float = 150e6,  # Bandwidth (150 MHz) - corrected from AIradar_dataset
                 chirp_duration: float = 20e-6,  # Chirp duration (20 μs) - corrected from AIradar_dataset
                 num_chirps: int = 128,  # Number of chirps - corrected from AIradar_dataset
                 samples_per_chirp: int = 1024,  # Samples per chirp - corrected from AIradar_dataset
                 num_rx: int = 2,
                 c: float = 3e8,  # Speed of light
                 R_max: float = 100,  # Maximum range (100 m)
                 zero_pad_factor: int = 8,  # Zero padding factor
                 enable_ofdm: bool = False,  # Enable OFDM communication
                 ofdm_config: Dict = None,  # OFDM configuration
                 save_directory: str = 'results'):  # Directory for saving files
        
        # Store basic parameters (following AIradar_dataset naming)
        self.fc = fc
        self.B = bandwidth  # Use B for bandwidth like AIradar_dataset
        self.T = chirp_duration  # Use T for chirp duration like AIradar_dataset
        self.Nc = num_chirps  # Use Nc for number of chirps like AIradar_dataset
        self.Ns = samples_per_chirp  # Use Ns for samples per chirp like AIradar_dataset
        self.num_rx = num_rx
        self.c = c
        self.R_max = R_max
        ...
        # Calculate derived parameters (following AIradar_dataset)
        self.lambda_c = c / fc
        self.slope = bandwidth / chirp_duration  # Chirp slope
        self.zero_pad = zero_pad_factor * samples_per_chirp
        
        # ✅ Sampling rate from max range (anti-aliasing) - corrected from AIradar_dataset
        self.fs = int(np.ceil((4 * bandwidth * R_max) / (chirp_duration * c)))
        
        # ✅ Doppler max velocity - corrected from AIradar_dataset
        self.v_max = self.lambda_c / (4 * self.T)
        
        # Time vectors - corrected from AIradar_dataset
        self.t_fast = np.arange(self.Ns) / self.fs
        self.t_slow = np.arange(self.Nc) * self.T
        self.t_chirp = self.t_fast  # Keep backward compatibility
        self.t_frame = self.t_slow  # Keep backward compatibility
        
        # ✅ Range axis using correct beat-to-range conversion - from AIradar_dataset
        range_res = (c * self.fs) / (2 * self.slope * self.zero_pad)
        self.range_axis = np.arange(self.zero_pad // 2) * range_res
        
        # ✅ Velocity axis from Doppler FFT - from AIradar_dataset
        ...
        self.max_range = self.range_axis[-1] if len(self.range_axis) > 0 else c * samples_per_chirp / (4 * bandwidth)
        ...
        print("\n=== Corrected ISAC Radar System Parameters ===")
    ...
    def generate_integrated_signal(self, targets: List[Dict], snr_db: float = 20) -> Tuple[np.ndarray, Dict, np.ndarray]:
        """
        ...
        radar_signal, rd_map = self.simulate_fmcw_signal(targets, snr_db)
        ...
        if self.enable_ofdm:
            ...
            if ofdm_signal is not None:
                ...
                # radar_signal is 2D (num_chirps, samples_per_chirp), so we need to handle it correctly
                for chirp_idx in range(min(self.num_chirps, len(ofdm_resized))):
                    if chirp_idx < radar_signal.shape[0]:
                        radar_signal[chirp_idx, :] += comm_power_ratio * ofdm_resized[chirp_idx, :]
        
        # Add noise
        integrated_signal = self.add_noise(radar_signal, snr_db)
        
        return integrated_signal, ground_truth, rd_map
    
    # Note: Range-Doppler map generation is now handled directly in simulate_fmcw_signal method
    # This follows the correct implementation from AIradar_dataset.py
    ...
    def visualize_range_doppler_map(self, rd_map: np.ndarray, targets: List[Dict], 
        ...
        Args:
            rd_map: Range-Doppler map [doppler_bins, range_bins]
            targets: List of target dictionaries
        ...
        if save_path:
            ...
            if not os.path.isabs(save_path):
                save_path = os.path.join(self.save_directory, save_path)
    ...
    def cfar_detection(self, rd_map):
        ...
        try:
            ...
            cfar_results = self._cfar_2d_custom(
                ...
                threshold_offset=5, # More sensitive threshold
...
class RadarDataset(Dataset):
    ...
    def __init__(self, num_samples: int = 1000, 
        ...
        for i in range(num_samples):
            ...
            # Generate signal
            clean_signal, rd_map = self.simulator.simulate_fmcw_signal(targets, snr_db=20)
            ...
            # noisy_signal is 2D (num_chirps, samples_per_chirp), need to add num_rx dimension
            # Input: [num_rx, num_chirps, samples_per_chirp, 2] (real, imag)
            # Expand to include num_rx dimension (simulate multiple receivers)
            noisy_signal_expanded = np.expand_dims(noisy_signal, axis=0)  # Add rx dimension
            noisy_signal_expanded = np.repeat(noisy_signal_expanded, self.simulator.num_rx, axis=0)
            input_signal = np.stack([noisy_signal_expanded.real, noisy_signal_expanded.imag], axis=-1)
            input_signal = torch.from_numpy(input_signal).float()
            
            # Label: [2, doppler_bins, range_bins] (magnitude, phase)
            # clean_signal is 2D (num_chirps, samples_per_chirp), use directly for fft2
            rd_complex = np.fft.fft2(clean_signal)
...
def plot_ber_curves(communication_performance: List[Dict], ai_communication_performance: List[Dict], save_dir: str = 'results'):
    ...
    for i, mod in enumerate(modulations):
        ...
        if mod == 'BPSK':
            ...
            # Default case for unknown modulation
            theoretical_ber = 0.1 * np.exp(-snr_range/10)
            ai_enhanced_ber = theoretical_ber * 0.6
...
def main():
    ...
    basic_signal, basic_ground_truth, basic_rd_map = basic_radar.generate_integrated_signal(test_targets, snr_db=20)
    ...
    for i, config in enumerate(ofdm_configs):
        ...
        integrated_signal, ground_truth, rd_map = isac_dataset.generate_integrated_signal(test_targets, snr_db=20)
        ...
        if ground_truth['communication_bits'] is not None:
            ...
            # integrated_signal is 2D (num_chirps, samples_per_chirp) and complex
            if integrated_signal.dtype == complex:
                complex_signal = integrated_signal[0, :] if len(integrated_signal.shape) == 2 else integrated_signal.flatten()
            else:
                # If real-valued, assume it's the real part only
                complex_signal = integrated_signal[0, :] if len(integrated_signal.shape) == 2 else integrated_signal.flatten()
