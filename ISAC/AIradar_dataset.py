...
try:
    ...
    print("Warning: visualization not available. Some visualizations will be skipped.")
...
class AIRadarDataset(Dataset):
    ...
    def _cfar_2d_custom(self, rd_map, num_train=8, num_guard=4, range_res=0.5, 
                       ...
                       threshold_offset=4, nms_kernel_size=3, max_detections=10):
        """
        ...
        for d_idx, r_idx in zip(doppler_idxs, range_idxs):
            ...
            candidates.append({
                ...
                "angle_deg": None,
                "magnitude": mag_db[d_idx, r_idx]
            })
        
        # ✅ Limit number of detections (sort by magnitude)
        candidates = sorted(candidates, key=lambda x: -x["magnitude"])
    ...
    def cfar_detection(self, rd_map):
        ...
        try:
            ...
            cfar_results = self._cfar_2d_custom(
                ...
                threshold_offset=5, #4, # More sensitive threshold (was 12 dB)
    ...
    def generate_dataset(self):
        ...
        # Evaluate CFAR detection performance
        print("\nEvaluating CFAR detection performance...")
        cfar_metrics = self.evaluate_cfar()
        
        print(f"\nCFAR Detection Performance Summary:")
        print(f"  Total Targets: {cfar_metrics['total_TP'] + cfar_metrics['total_FN']}")
        print(f"  Total Detections: {cfar_metrics['total_TP'] + cfar_metrics['total_FP']}")
        print(f"  True Positives: {cfar_metrics['total_TP']}")
        print(f"  False Positives: {cfar_metrics['total_FP']}")
        print(f"  False Negatives: {cfar_metrics['total_FN']}")
        print(f"  Overall Precision: {cfar_metrics['overall_precision']:.3f}")
        print(f"  Overall Recall: {cfar_metrics['overall_recall']:.3f}")
        print(f"  Overall F1-Score: {cfar_metrics['overall_f1_score']:.3f}")
        print(f"  Detection Rate: {cfar_metrics['detection_rate']:.1f}%")
        print(f"  False Alarm Rate: {cfar_metrics['false_alarm_rate']:.2f} per sample")
        print(f"\nLocalization Error Metrics:")
        print(f"  Mean Range Error: {cfar_metrics['overall_mean_range_error']:.2f} m")
        print(f"  Mean Velocity Error: {cfar_metrics['overall_mean_velocity_error']:.2f} m/s")
        print(f"  Range RMSE: {cfar_metrics['overall_rmse_range']:.2f} m")
        print(f"  Velocity RMSE: {cfar_metrics['overall_rmse_velocity']:.2f} m/s")
        print(f"  Range Std Dev: {cfar_metrics['overall_std_range_error']:.2f} m")
        print(f"  Velocity Std Dev: {cfar_metrics['overall_std_velocity_error']:.2f} m/s")
        
        # Store metrics for later access
        self.cfar_performance_metrics = cfar_metrics
    ...
    def evaluate_cfar(self, tol=2):
        """
        Evaluate CFAR detection performance against ground truth targets for the entire dataset.
        
        Args:
            tol: Tolerance for matching detections to ground truth (default: 2)
        ...
        Returns:
            ...
            - detection_rate: Percentage of targets detected
            - false_alarm_rate: Average false alarms per sample
            - sample_metrics: List of per-sample metrics
        """
        if not hasattr(self, 'cfar_detections') or not hasattr(self, 'target_info'):
            raise ValueError("CFAR detections and target info not available. Run generate_dataset first.")
        ...
        for i, (cfar_results, info) in enumerate(zip(self.cfar_detections, self.target_info)):
            targets = info['targets']
            detections = cfar_results
            ...
            for target in targets:
                ...
                range_idx = np.argmin(np.abs(self.range_axis - target['range']))
                velocity_idx = np.argmin(np.abs(self.velocity_axis - target['velocity']))
                target_indices.append((range_idx, velocity_idx))
            
            # Check each target for detection and calculate localization errors
            detected_targets = set()
            range_errors = []
            velocity_errors = []
            
            for target_idx, (r_gt, d_gt) in enumerate(target_indices):
                # Find the closest matching detection
                best_detection = None
                min_distance = float('inf')
                
                for detection in detections:
                    distance = np.sqrt((detection['range_idx'] - r_gt)**2 + (detection['doppler_idx'] - d_gt)**2)
                    if distance <= tol * np.sqrt(2) and distance < min_distance:
                        min_distance = distance
                        best_detection = detection
                
                if best_detection is not None:
                    sample_TP += 1
                    detected_targets.add(target_idx)
                    
                    # Calculate localization errors
                    target = targets[target_idx]
                    range_error = abs(best_detection['range_m'] - target['range'])
                    velocity_error = abs(best_detection['velocity_mps'] - target['velocity'])
                    range_errors.append(range_error)
                    velocity_errors.append(velocity_error)
                else:
                    sample_FN += 1
            
            # Count false positives (detections not matching any target)
            for detection in detections:
                is_false_positive = True
                for r_gt, d_gt in target_indices:
                    if abs(detection['range_idx'] - r_gt) <= tol and abs(detection['doppler_idx'] - d_gt) <= tol:
            ...
            # Calculate localization error statistics for this sample
            ...
            std_range_error = np.std(range_errors) if range_errors else 0.0
            std_velocity_error = np.std(velocity_errors) if velocity_errors else 0.0
            ...
            sample_metrics.append({
                ...
                'std_range_error': std_range_error,
            ...
            total_FP += sample_FP
            total_FN += sample_FN
            all_range_errors.extend(range_errors)
            all_velocity_errors.extend(velocity_errors)
        
        # Calculate overall metrics
        overall_precision = total_TP / (total_TP + total_FP) if (total_TP + total_FP) > 0 else 0
        overall_recall = total_TP / (total_TP + total_FN) if (total_TP + total_FN) > 0 else 0
        overall_f1 = 2 * overall_precision * overall_recall / (overall_precision + overall_recall) if (overall_precision + overall_recall) > 0 else 0
        detection_rate = (total_TP / (total_TP + total_FN)) * 100 if (total_TP + total_FN) > 0 else 0
        false_alarm_rate = total_FP / len(self.cfar_detections) if len(self.cfar_detections) > 0 else 0
        
        # Calculate overall localization error statistics
        overall_mean_range_error = np.mean(all_range_errors) if all_range_errors else 0.0
        overall_mean_velocity_error = np.mean(all_velocity_errors) if all_velocity_errors else 0.0
        overall_std_range_error = np.std(all_range_errors) if all_range_errors else 0.0
        overall_std_velocity_error = np.std(all_velocity_errors) if all_velocity_errors else 0.0
        overall_rmse_range = np.sqrt(np.mean(np.array(all_range_errors)**2)) if all_range_errors else 0.0
        overall_rmse_velocity = np.sqrt(np.mean(np.array(all_velocity_errors)**2)) if all_velocity_errors else 0.0
        ...
        return {
            ...
            'overall_mean_range_error': overall_mean_range_error,
            'overall_mean_velocity_error': overall_mean_velocity_error,
            'overall_std_range_error': overall_std_range_error,
            'overall_std_velocity_error': overall_std_velocity_error,
            'overall_rmse_range': overall_rmse_range,
            'overall_rmse_velocity': overall_rmse_velocity,
    ...
    def _load_data(self, datapath):
        ...
        with h5py.File(datapath, 'r') as f:
            self.time_domain_data = f['time_domain_data'][:] #(1000, 128, 1024, 2)
            self.range_doppler_maps = f['range_doppler_maps'][:] #(1000, 128, 4096)
            self.target_masks = f['target_masks'][:] #(1000, 128, 4096, 1)
    ...
    def __getitem__(self, idx):
        ...
        has_cfar= False
        if hasattr(self, 'cfar_detections') and self.cfar_detections is not None:
            if idx < len(self.cfar_detections):
                has_cfar= True
        ...
        return {
            ...
            'cfar_detections': self.cfar_detections[idx] if has_cfar else [],
...
if __name__ == "__main__":
    ...
    dataset = AIRadarDataset(
        num_samples=10,
        drawfig=True,
        save_path='output/radar_corrected_test1'
    )
